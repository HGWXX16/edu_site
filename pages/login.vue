  ...
  methods: {
    ...
    submitLogin() {
      ...
      loginApi.submitLogin(this.user).then(response => {
        // 将jwt写入cookie
        cookie.set('yao_jwt_token', response.data.token, { domain: 'localhost' })
        // debugger
        // 跳转到网站的首页面
        ...
        if (document.referrer.indexOf('register') !== -1) { // 如果上一页是注册页面，则跳转到网站的首页
          ...
          // console.log('cookie before:' + cookie.get('yao_jwt_token'))
          history.go(-1) // 跳转到上一个刚刚访问的页面
          // console.log('cookie after:' + cookie.get('yao_jwt_token'))
        }
      })
    }
  }
